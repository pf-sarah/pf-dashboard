...
function buildDefaultGeorgiaSchedule(): WeekSchedule[] {
  return Array.from({ length: WEEKS }, () => ({ 'ga-mgr': 0, 'ga-1': 0, 'ga-2': 0 }));
}
...
export default function SchedulePage() {
  ...
  const hasRates    = designers.some(d => (d.payType === 'hourly' && d.hourlyRate > 0) || (d.payType === 'salary' && d.annualSalary > 0));
          ...
          {showRoster && (
            <div className="mt-3 bg-white border border-slate-100 rounded-xl p-5">
              ...
              {deletedStack.length > 0 && (
                <button onClick={handleUndo} className="mt-3 text-xs px-3 py-1 border border-amber-200 rounded text-amber-600 hover:bg-amber-50 transition-colors">
        ...
        {activeTab === 'queue' && (
          <div className="space-y-6">
            <div className="bg-white border border-slate-100 rounded-xl p-5">
              ...
              <p className="text-xs text-amber-600 mb-4">Under 8 weeks total = overstaffed (designing faster than flowers can dry).</p>
              <div className="space-y-2.5">
                {futureTurnarounds.slice(0, 20).map((total, w) => {
            ...
            <div className="bg-white border border-slate-100 rounded-xl overflow-hidden">
              <div className="px-5 py-3 border-b border-slate-100">
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  FIFO queue drain based on {location} designable queue of {designableQueue} orders and scheduled capacity.
                </p>
              </div>
              {location !== 'Utah' ? (
                        ...
                        return (
                          <tr key={i} className={`border-b border-slate-50 ${
                            ...
                            <td className="px-3 py-2 text-right">
                              {done ? (
                                ...
                                <span className="text-green-700 text-[10px] bg-green-100 rounded px-1.5 py-0.5">
                                  still drying — {'preservationWeeksLeft' in row ? row.preservationWeeksLeft : 0} wks left
                                </span>
                              ) : weeksLeft === 0 ? (
                            ...
                            <td className="px-3 py-2">
                              {done ? (
                                ...
                                <span className="text-xs text-slate-400 italic">
                                  enters design queue in ~{'preservationWeeksLeft' in row ? row.preservationWeeksLeft : 0} wk{('preservationWeeksLeft' in row && row.preservationWeeksLeft === 1) ? '' : 's'},
                                  then {weeksLeft !== null ? `~${weeksLeft - ('preservationWeeksLeft' in row ? row.preservationWeeksLeft : 0)} wks in design queue` : 'queue estimate pending'}
                                </span>
                              ) : weeksLeft === null ? (
                            ...
                            <td className="px-3 py-2 text-center">
                              ...
                              ) : done ? <span className="text-xs text-slate-300">—</span> : <span className="text-xs text-slate-400">TBD</span>}
                            </td>
                            <td className="px-3 py-2 text-center">
