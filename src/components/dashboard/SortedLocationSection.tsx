...
interface LocationData {
  ...
  UtahOrders:    Record<string, OrderEntry[]>;
  GeorgiaOrders: Record<string, OrderEntry[]>;
  unresolved:    number;
  cachedCount:   number;
}
...
function OrderPanel({
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter(o =>
      ...
      (o.variantTitle ?? '').toLowerCase().includes(q) ||
    ...
    <div className="mt-2 rounded-lg border border-slate-200 bg-white overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-100 bg-slate-50">
        <span className="text-xs font-semibold text-slate-600">
          {STATUS_LABELS[status] ?? status} — {orders.length} orders
        </span>
      ...
      <div className="px-3 py-2 border-b border-slate-100">
        <input
          type="text"
          placeholder="Search order #, frame, staff…"
          value={search}
        ...
        <table className="min-w-full text-xs">
          ...
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr key={`${o.orderNum}|${o.variantTitle}|${i}`}
  ...
  const globalResults = useMemo(() => {
    ...
    (['Utah', 'Georgia'] as const).forEach(loc => {
      ...
      Object.entries(ordersMap ?? {}).forEach(([status, orders]) => {
        orders.forEach(o => {
          if (o.orderNum.toLowerCase().includes(q) ||
              (o.variantTitle ?? '').toLowerCase().includes(q)) {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="relative">
        <input
          type="text"
          placeholder="Search any order number across all locations and statuses…"
          value={globalSearch}
        ...
        {globalSearch && (
          <button onClick={() => setGlobalSearch('')}
            className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 text-lg">×</button>
        )}
      </div>
        ...
        <div className="rounded-lg border border-slate-200 bg-white overflow-hidden">
          ...
          {globalResults.length === 0 ? (
            <p className="px-4 py-4 text-xs text-slate-400 italic">
              No orders found. Note: only orders synced to Supabase appear here. Run Sync Now if recently updated.
            </p>
          ) : (
