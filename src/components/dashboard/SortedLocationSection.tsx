...
interface OrderRow {
  ...
  orderDate: string; eventDate: string; staff: string; enteredAt: string; location: string;
}
...
function OrderPanel({status,location,onClose}:{status:string;location:string;onClose:()=>void}) {
  ...
  useEffect(()=>{
    ...
    fetch(`/api/pipeline-orders?status=${status}&location=${encodeURIComponent(location)}`)
      .then(r=>r.json()).then(d=>{if(d.error){setError(d.error);return;}setOrders(d.orders??[]);})
      .catch(e=>setError(String(e))).finally(()=>setLoading(false));
  },[status,location]);
  const filtered=useMemo(()=>{
    const q=search.trim().toLowerCase();if(!q)return orders;
    return orders.filter(o=>o.num.includes(q)||o.name.toLowerCase().includes(q)||o.variant.toLowerCase().includes(q)||o.staff.toLowerCase().includes(q));
  },[orders,search]);
  return (
    <div className="mt-2 rounded-lg border border-slate-200 bg-white overflow-hidden">
      <div className="flex items-center justify-between px-3 py-2 border-b border-slate-100 bg-slate-50">
        <span className="text-xs font-semibold text-slate-600">{STATUS_LABELS[status]??status}{!loading&&` — ${orders.length} orders`}</span>
        <button onClick={onClose} className="text-slate-400 hover:text-slate-600 text-lg leading-none">×</button>
      </div>
      {loading?(
        <p className="px-4 py-4 text-xs text-slate-400 italic">Loading orders… (scanning up to 48 months of history)</p>
      ):error?(
        <p className="px-4 py-3 text-xs text-red-500">{error}</p>
      ):(
        <>
          <div className="px-3 py-2 border-b border-slate-100">
            <input type="text" placeholder="Search order #, customer, frame, staff…" value={search} onChange={e=>setSearch(e.target.value)}
              className="w-full border border-slate-200 rounded px-2 py-1.5 text-xs text-slate-700 bg-white placeholder:text-slate-300 focus:outline-none focus:ring-1 focus:ring-indigo-300" autoFocus />
          ...
          <div className="overflow-auto max-h-72">
            <table className="min-w-full text-xs">
              ...
              <tbody>
                {filtered.length===0?(
                  <tr><td colSpan={6} className="px-3 py-4 text-center text-slate-400 italic">No results</td></tr>
                ):filtered.map((o,i)=>(
                  <tr key={`${o.num}|${o.variant}|${i}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50">
                    ...
                    <div className="flex justify-between items-center text-xs">
                      ...
                      {count>0?(
                        <button onClick={()=>setExpandedStatus(prev=>prev===st?null:st)}
                          ...
                          title="Click to see orders">{count.toLocaleString()}</button>
                      ):(
                        <span className="text-slate-300 font-semibold text-xs">0</span>
                      )}
                    </div>
                    {isExpanded&&<OrderPanel status={st} location={name} onClose={()=>setExpandedStatus(null)} />}
                  </div>
                );
              })}
            </CardContent>
